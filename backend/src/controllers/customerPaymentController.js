...
exports.deleteCustomerPayment = async (req, res) => {
  ...
  try {
    ...
    if (entry.payment_method === 'bank' && entry.bank_id) {
      ...
      if (bank) {
        // Reverse bank balance (subtract the amount that was added)
        ...
        // Create reversal bank transaction (OUTGOING)
        await BankTransaction.create({
          bank_id: bank.id,
          transaction_type: 'out',
          amount: paymentAmount.toFixed(2),
          description: `Reversal of payment from customer`,
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 2: Create reversal ledger entry (CREDIT to restore balance)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // STEP 3: Update cheque record if exists
    // ═══════════════════════════════════════════════════════════════════════
    if (entry.cheque_number && entry.reference_id) {
      await Cheque.update(
        ...
        { where: { id: entry.reference_id }, transaction: dbTransaction }
      );
    }
