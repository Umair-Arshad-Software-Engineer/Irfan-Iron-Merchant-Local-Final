...
// //
// //   SaleType _selectedSaleType = SaleType.filled;
// //
...
// //   bool _showCartPanel = true; // For mobile: toggle cart visibility
// //
// //   @override
// //   void initState() {
// //     super.initState();
...
// //             color: Colors.white,
// //             border: Border(
// //               top: BorderSide(color: const Color(0xFFEEEEF5)),
// //             ),
// //           ),
// //           child: Row(
// //             children: [
...
// //                   onPressed: () => setState(() => _showCartPanel = !_showCartPanel),
// //                   icon: Icon(
// //                     _showCartPanel ? Icons.keyboard_arrow_down : Icons.keyboard_arrow_up,
// //                     size: 18,
// //                   ),
// //                   label: Text(
// //                     _showCartPanel ? 'Hide Cart' : 'Show Cart (${_cartItems.length})',
...
// //             ),
// //           ),
// //           child: _showCartPanel
// //               ? _buildCartPanel(isPOS: _isPosMode)
...
// //           ),
// //           if (!_isMobile) const SizedBox(width: 15),
// //           Expanded(
// //             child: Column(
// //               crossAxisAlignment: CrossAxisAlignment.start,
// //               children: [
...
// //           ),
// //         ),
// //         if (!_isMobile) _buildCategoryChips(),
// //         if (!_isMobile && _selectedCategory != null) _buildSubcategoryChips(),
...
// //         autofocus: _isPosMode && !_isMobile,
// //         decoration: InputDecoration(
// //           hintText: _isMobile
// //               ? 'Search products…'
// //               : 'Scan barcode or search by product name / barcode…',
// //           hintStyle:
...
// //         maxCrossAxisExtent: _isMobile ? 150 : 200,
// //         crossAxisSpacing: 10,
// //         mainAxisSpacing: 10,
...
// //         previousBalance: _selectedCustomer?.balance ?? 0.0,
// //       );
// //
// //       if (mounted) Navigator.pop(context);
...
//     if (combinations.isEmpty) {
//       ScaffoldMessenger.of(context).showSnackBar(
//         SnackBar(content: Text('No length combinations available for ${product.itemName}'), backgroundColor: Colors.orange),
//       );
//       return;
//     }
//
//     List<String> currentSelections = List.from(item.selectedLengths);
//     Map<String, double> currentQuantities = Map.from(item.lengthQuantities);
//     final weightController = TextEditingController(text: item.weight > 0 ? item.weight.toStringAsFixed(2) : '');
//
//     final Map<String, TextEditingController> qtyControllers = {};
//     for (final combo in combinations) {
//       final qty = currentQuantities[combo.length] ?? 0.0;
...
//                 crossAxisAlignment: CrossAxisAlignment.start,
//                 children: [
//                   Text('Select Lengths — ${product.itemName}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
//                   const SizedBox(height: 2),
//                   Text('${combinations.length} length${combinations.length != 1 ? 's' : ''} available', style: const TextStyle(fontSize: 11, color: Color(0xFF9CA3AF))),
//                 ],
//               ),
//               content: SizedBox(
//                 width: double.maxFinite,
...
//                     const Align(
//                       alignment: Alignment.centerLeft,
//                       child: Text('Lengths & Quantities', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
//                     ),
//                     const SizedBox(height: 6),
//                     Expanded(
//                       child: ListView.builder(
...
//                                   subtitle: combo.lengthDecimal.isNotEmpty ? Text('Decimal: ${combo.lengthDecimal}', style: const TextStyle(fontSize: 10)) : null,
//                                   value: isSelected,
//                                   activeColor: const Color(0xFF10B981),
//                                   onChanged: (val) {
//                                     setDlgState(() {
...
//                           mainAxisAlignment: MainAxisAlignment.spaceBetween,
//                           children: [
//                             Text('${currentSelections.length} selected', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
//                             Text('${currentQuantities.values.fold(0.0, (s, q) => s + q).toStringAsFixed(0)} pcs', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600)),
//                           ],
//                         ),
//                       ),
//                   ],
...
//                     for (final combo in combinations) {
//                       if (currentSelections.contains(combo.length)) {
//                         currentQuantities[combo.length] = double.tryParse(qtyControllers[combo.length]?.text ?? '') ?? 1.0;
//                       }
//                     }
//                     final String lengthsDisplay = currentSelections.map((l) {
//                       final q = currentQuantities[l] ?? 1.0;
...
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(content: Text('Could not fetch customer prices: $e'), backgroundColor: const Color(0xFFEF4444)),
//         );
//       }
//     } finally {
//       if (mounted) setState(() => _isFetchingCustomerPrices = false);
...
//         CircularProgressIndicator(color: Color(0xFF7C3AED)),
//         SizedBox(height: 12),
//         Text('Loading sale data…', style: TextStyle(color: Color(0xFF9CA3AF), fontSize: 13)),
//       ]))
//           : Column(
//         children: [
//           _buildHeader(),
...
//             child: _isMobile
//                 ? _buildMobileLayout()
//                 : (_isPosMode ? _buildPosLayout() : _buildInvoiceLayout()),
//           ),
//         ],
//       ),
//     );
...
//     return Column(
//       children: [
//         // Toggle between product panel and cart
//         Container(
//           padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
//           decoration: BoxDecoration(
//             color: Colors.white,
//             border: Border(bottom: BorderSide(color: const Color(0xFFEEEEF5))),
//           ),
//           child: Row(
//             children: [
//               _buildMobileTab('Products', Icons.inventory_2, _showProductPanel, () {
//                 setState(() => _showProductPanel = true);
//               }),
//               _buildMobileTab('Cart (${_cartItems.length})', Icons.shopping_cart, !_showProductPanel, () {
//                 setState(() => _showProductPanel = false);
//               }),
//             ],
//           ),
//         ),
//         Expanded(
//           child: _showProductPanel
//               ? _buildProductPanelCompact()
//               : _buildCartPanelCompact(),
//         ),
//       ],
//     );
//   }
...
//             mainAxisAlignment: MainAxisAlignment.center,
//             children: [
//               Icon(icon, size: 16, color: active ? const Color(0xFF7C3AED) : const Color(0xFF9CA3AF)),
//               const SizedBox(width: 4),
...
//   }
//
//   Widget _buildProductPanelCompact() {
//     return Column(
//       children: [
//         Padding(
...
//             child: _buildSearchDropdownCompact(),
//           ),
//         if (_searchController.text.isEmpty)
//           Expanded(
//             child: _isLoadingProducts
//                 ? const Center(child: CircularProgressIndicator())
//                 : _buildBrowseProductGridCompact(),
//           ),
//       ],
//     );
//   }
...
//   }
//
//   Widget _buildSearchDropdownCompact() {
//     return Container(
...
//             ),
//             title: Text(p.itemName, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
//             subtitle: Text(p.barcode ?? '', style: const TextStyle(fontSize: 10, color: Color(0xFF9CA3AF))),
//             trailing: Row(
//               mainAxisSize: MainAxisSize.min,
//               children: [
//                 Text('Rs ${displayPrice.toStringAsFixed(2)}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF7C3AED))),
//                 const SizedBox(width: 6),
//                 inCart
//                     ? Container(padding: const EdgeInsets.all(3), decoration: BoxDecoration(color: const Color(0xFFECFDF5), borderRadius: BorderRadius.circular(4)),
...
//   }
//
//   Widget _buildBrowseProductGridCompact() {
//     final products = _filteredBrowseProducts;
//     if (products.isEmpty) {
//       return const Center(
//         child: Column(
//           mainAxisAlignment: MainAxisAlignment.center,
//           children: [
//             Icon(Icons.point_of_sale, size: 40, color: Color(0xFFD1D5DB)),
//             SizedBox(height: 8),
//             Text('No products', style: TextStyle(fontSize: 13, color: Color(0xFF9CA3AF))),
//           ],
//         ),
//       );
//     }
...
//       ),
//       itemCount: products.length,
//       itemBuilder: (ctx, i) => _buildProductCardCompact(products[i]),
//     );
//   }
//
//   Widget _buildProductCardCompact(ProductModel product) {
//     final inCart = _cartItems.any((item) => item.product.id == product.id);
//     final displayPrice = _resolvePrice(product);
//     final isLowStock = product.physicalQty <= product.minStock;
//
...
//                         child: const Text('Low', style: TextStyle(fontSize: 7, fontWeight: FontWeight.bold, color: Color(0xFF92400E))),
//                       ),
//                     ),
//                   if (inCart)
//                     Positioned(
...
//                     Text(
//                       product.itemName,
//                       style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Color(0xFF1E1E2D)),
//                       maxLines: 2,
//                       overflow: TextOverflow.ellipsis,
//                     ),
//                     const Spacer(),
...
//   }
//
//   Widget _buildCartPanelCompact() {
//     return Column(
//       children: [
//         Expanded(
//           child: _cartItems.isEmpty
//               ? const Center(child: Text('Cart is empty', style: TextStyle(fontSize: 13, color: Color(0xFF9CA3AF))))
//               : ListView.builder(
//             padding: const EdgeInsets.all(6),
//             itemCount: _cartItems.length,
//             itemBuilder: (ctx, i) => _buildCartItemCompact(i),
//           ),
//         ),
//         Container(
//           padding: const EdgeInsets.all(8),
//           decoration: BoxDecoration(
//             color: Colors.white,
//             border: Border(top: BorderSide(color: const Color(0xFFEEEEF5))),
//           ),
//           child: _buildSummaryCompact(),
//         ),
//       ],
//     );
//   }
//
//   Widget _buildCartItemCompact(int index) {
//     final item = _cartItems[index];
//     final isSaryaType = _selectedSaleType == SaleType.sarya;
//
//     return Container(
...
//                   child: Row(
//                     children: [
//                       _weightBtnCompact(Icons.remove, () {
//                         setState(() {
//                           final newWeight = (item.weight - 0.5).clamp(0.0, 9999.0);
//                           final old = _cartItems[index];
//                           _cartItems[index] = SaleItem(
...
//                       Expanded(
//                         child: TextField(
//                           controller: TextEditingController(text: item.weight.toStringAsFixed(2)),
//                           keyboardType: const TextInputType.numberWithOptions(decimal: true),
//                           textAlign: TextAlign.center,
//                           style: const TextStyle(fontSize: 11),
//                           decoration: InputDecoration(
...
//                   child: Row(
//                     children: [
//                       _qtyBtnCompact(Icons.remove, () => _updateQty(index, -1)),
//                       Expanded(
//                         child: TextField(
//                           controller: TextEditingController(text: item.quantity.toString()),
//                           keyboardType: TextInputType.number,
...
//                         ),
//                       ),
//                       _qtyBtnCompact(Icons.add, () => _updateQty(index, 1)),
//                     ],
//                   ),
//                 ),
//               const SizedBox(width: 6),
...
//                   final qty = item.lengthQuantities[length] ?? 1.0;
//                   return Chip(
//                     label: Text(_safeLengthLabel(length, qty), style: const TextStyle(fontSize: 9)),
...
//                 crossAxisAlignment: CrossAxisAlignment.start,
//                 children: [
//                   Text('Total: Rs ${_grandTotal.toStringAsFixed(2)}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
//                   if (_discountValue > 0)
//                     Text('Disc: -Rs ${_discountValue.toStringAsFixed(2)}', style: const TextStyle(fontSize: 10, color: Color(0xFF10B981))),
//                   if (customCount > 0)
//                     Text('${customCount} custom price', style: const TextStyle(fontSize: 9, color: Color(0xFF065F46))),
//                 ],
//               ),
//             ),
//             if (_cartItems.isNotEmpty && _selectedCustomer != null)
//               ElevatedButton(
//                 onPressed: _isEditMode ? _submitEdit : (_isPosMode ? _processPayment : _createInvoice),
//                 style: ElevatedButton.styleFrom(
//                   backgroundColor: const Color(0xFF7C3AED),
//                   foregroundColor: Colors.white,
//                   padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
...
//                 ),
//                 child: Text(_isEditMode ? 'Save' : (_isPosMode ? 'Pay' : 'Invoice'), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
//               ),
//           ],
//         ),
//       ],
...
//   }
//
//   Widget _weightBtnCompact(IconData icon, VoidCallback onTap) => GestureDetector(
//     onTap: onTap,
//     child: Container(
//       width: 24,
//       height: 24,
//       decoration: BoxDecoration(color: const Color(0xFFEFF6FF), borderRadius: BorderRadius.circular(4)),
//       child: Icon(icon, size: 12, color: const Color(0xFF3B82F6)),
//     ),
//   );
//
//   Widget _qtyBtnCompact(IconData icon, VoidCallback onTap) => GestureDetector(
//     onTap: onTap,
//     child: Container(
//       width: 24,
//       height: 24,
//       decoration: BoxDecoration(color: const Color(0xFFF3F0FF), borderRadius: BorderRadius.circular(4)),
//       child: Icon(icon, size: 12, color: const Color(0xFF7C3AED)),
//     ),
//   );
//
//   // ── HEADER ───────────────────────────────────
...
//           Expanded(
//             child: Text(
//               _isEditMode ? 'Edit ${widget.existingSale!.invoiceNumber}' : 'Sales',
//               style: TextStyle(fontSize: _isMobile ? 16 : 20, fontWeight: FontWeight.bold, color: const Color(0xFF1E1E2D)),
//             ),
//           ),
//           if (!_isMobile) ...[
//             if (!_isEditMode) ...[
...
//                 child: Row(
//                   children: [
//                     _buildToggleBtn('POS', Icons.point_of_sale, true),
//                     _buildToggleBtn('Inv', Icons.receipt_long, false),
//                   ],
//                 ),
//               ),
//               const SizedBox(width: 8),
...
//               label: const Text('Clear', style: TextStyle(color: Color(0xFFEF4444), fontSize: 11)),
...
//         child: Row(
//           children: [
//             Icon(type == SaleType.sarya ? Icons.scale : Icons.production_quantity_limits, size: 12, color: isActive ? const Color(0xFF7C3AED) : const Color(0xFF9CA3AF)),
//             const SizedBox(width: 4),
//             Text(type.displayName, style: TextStyle(fontSize: 11, fontWeight: isActive ? FontWeight.w600 : FontWeight.w500, color: isActive ? const Color(0xFF7C3AED) : const Color(0xFF9CA3AF))),
//           ],
//         ),
//       ),
//     );
...
//             const SizedBox(width: 4),
//             Text(label, style: TextStyle(fontSize: 11, fontWeight: isActive ? FontWeight.w600 : FontWeight.w500, color: isActive ? const Color(0xFF7C3AED) : const Color(0xFF9CA3AF))),
//           ],
//         ),
//       ),
//     );
...
//           child: Column(
//             children: [
//               Expanded(child: _buildProductPanelCompactWeb()),
//               _buildLeftOptionsPanel(),
//             ],
//           ),
//         ),
...
//           decoration: const BoxDecoration(
//             color: Colors.white,
//             border: Border(left: BorderSide(color: Color(0xFFEEEEF5), width: 1)),
//           ),
//           child: _buildCartPanelWeb(isPOS: true),
//         ),
//       ],
//     );
//   }
...
//             child: Column(
//               children: [
//                 _buildInvoiceMetaCompact(),
//                 const SizedBox(height: 8),
//                 _buildSearchBarCompact(),
//                 const SizedBox(height: 6),
//                 if (_searchResults.isNotEmpty) _buildSearchDropdownCompact(),
//                 const SizedBox(height: 8),
//                 _buildInvoiceItemsTableCompact(),
//                 const SizedBox(height: 8),
//                 _buildInvoiceOptionsCardCompact(),
//                 const SizedBox(height: 8),
//                 _buildInvoiceNotesCompact(),
//               ],
//             ),
//           ),
//         ),
...
//             border: Border(left: BorderSide(color: Color(0xFFEEEEF5), width: 1)),
//           ),
//           child: _buildCartPanelWeb(isPOS: false),
//         ),
//       ],
//     );
//   }
//
//   Widget _buildProductPanelCompactWeb() {
//     return Column(
//       children: [
//         Padding(
...
//               padding: const EdgeInsets.symmetric(horizontal: 12),
//               children: [
//                 _filterChipCompact(label: 'All', selected: _selectedCategory == null, onTap: () => setState(() { _selectedCategory = null; _selectedSubcategory = null; })),
//                 ..._categories.map((cat) => _filterChipCompact(label: cat, selected: _selectedCategory == cat, onTap: () => setState(() { _selectedCategory = _selectedCategory == cat ? null : cat; _selectedSubcategory = null; }))),
//               ],
//             ),
//           ),
...
//                 padding: const EdgeInsets.symmetric(horizontal: 12),
//                 children: [
//                   _filterChipCompact(label: 'All sub', selected: _selectedSubcategory == null, onTap: () => setState(() => _selectedSubcategory = null), small: true),
//                   ..._subcategories.map((sub) => _filterChipCompact(label: sub, selected: _selectedSubcategory == sub, onTap: () => setState(() { _selectedSubcategory = _selectedSubcategory == sub ? null : sub; }), small: true)),
//                 ],
//               ),
//             ),
//         ],
...
//         Expanded(
//           child: _searchController.text.isNotEmpty
//               ? (_searchResults.isEmpty && !_isSearching ? _buildNoResultsCompact() : const SizedBox.shrink())
//               : _buildBrowseProductGridCompact(),
//         ),
//       ],
//     );
//   }
//
//   Widget _filterChipCompact({required String label, required bool selected, required VoidCallback onTap, bool small = false}) {
//     return Padding(
...
//           ),
//           child: Text(label, style: TextStyle(fontSize: small ? 9 : 10, fontWeight: selected ? FontWeight.w600 : FontWeight.w400, color: selected ? const Color(0xFF7C3AED) : const Color(0xFF6B7280))),
//         ),
//       ),
//     );
//   }
//
//   Widget _buildNoResultsCompact() => const Center(
//     child: Column(
//       mainAxisAlignment: MainAxisAlignment.center,
//       children: [
//         Icon(Icons.search_off, size: 32, color: Color(0xFFD1D5DB)),
//         SizedBox(height: 8),
//         Text('No products found', style: TextStyle(fontSize: 13, color: Color(0xFF9CA3AF))),
//       ],
//     ),
//   );
//
...
//               child: Row(
//                 children: [
//                   Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: const Color(0xFFF3F0FF), borderRadius: BorderRadius.circular(6)),
//                       child: const Icon(Icons.tune, size: 14, color: Color(0xFF7C3AED))),
//                   const SizedBox(width: 6),
//                   const Text('Discount & Pricing', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
//                   if (activeOptions > 0) ...[
//                     const SizedBox(width: 4),
//                     Container(padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1), decoration: BoxDecoration(color: const Color(0xFF7C3AED), borderRadius: BorderRadius.circular(8)),
...
//                 crossAxisAlignment: CrossAxisAlignment.start,
//                 children: [
//                   Expanded(child: _buildCompactDiscountSection(usingCustomerDiscount, hasCustomerDiscount)),
//                   const SizedBox(width: 8),
//                   if (hasCustomer) Expanded(child: _buildCompactCustomerPriceToggle()),
//                 ],
//               ),
//             ),
...
//   }
//
//   Widget _buildCompactDiscountSection(bool usingCustomerDiscount, bool hasCustomerDiscount) {
//     return Column(
//       crossAxisAlignment: CrossAxisAlignment.start,
//       children: [
//         Row(
//           children: [
//             const Text('Discount', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
//             const Spacer(),
//             GestureDetector(
...
//                 child: Text(_usePercentDiscount ? '%' : 'Rs', style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: Color(0xFF7C3AED))),
//               ),
//             ),
//           ],
//         ),
...
//       crossAxisAlignment: CrossAxisAlignment.start,
//       children: [
//         const Text('Customer Pricing', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Color(0xFF374151))),
//         const SizedBox(height: 4),
//         AnimatedContainer(
//           duration: const Duration(milliseconds: 200),
...
//                 child: Text(
//                   'Use Prices',
//                   style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: _useCustomerPrices ? const Color(0xFF065F46) : const Color(0xFF374151)),
//                 ),
//               ),
//               if (_isFetchingCustomerPrices)
...
//   }
//
//   Widget _buildInvoiceOptionsCardCompact() {
//     final hasCustomer = _selectedCustomer != null;
//     final hasCustomerDiscount = hasCustomer && _selectedCustomer!.discountPercent > 0;
//     final bool usingCustomerDiscount = hasCustomerDiscount && _usePercentDiscount && _discountPercent == _selectedCustomer!.discountPercent;
//
...
//           Expanded(
//             child: Text(
//               usingCustomerDiscount ? 'Applied ${_selectedCustomer!.discountPercent.toStringAsFixed(1)}%' : '${_selectedCustomer!.discountPercent.toStringAsFixed(1)}% for ${_selectedCustomer!.name}',
//               style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: usingCustomerDiscount ? const Color(0xFF065F46) : const Color(0xFF374151)),
//             ),
//           ),
//         ],
//       ),
...
//
//   // ── INVOICE WIDGETS ───────────────────────────
//   Widget _buildInvoiceMetaCompact() {
//     return Container(
//       padding: const EdgeInsets.all(10),
//       decoration: BoxDecoration(
//         color: Colors.white,
...
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
//           const Text('Invoice Details', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
//           const SizedBox(height: 8),
//           _buildReferenceFieldCompact(),
//           const SizedBox(height: 8),
//           Row(
//             children: [
//               Expanded(child: _buildMetaFieldCompact(label: 'Date', value: _formatDate(_invoiceDate), icon: Icons.calendar_today, onTap: () async {
...
//               })),
//               const SizedBox(width: 8),
//               Expanded(child: _buildMetaFieldCompact(label: 'Due', value: _dueDate != null ? _formatDate(_dueDate!) : 'Not set', icon: Icons.event, onTap: () async {
...
//   }
//
//   Widget _buildReferenceFieldCompact() {
//     return Container(
//       padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
//       decoration: BoxDecoration(
//         color: const Color(0xFFF9FAFB),
...
//               decoration: const InputDecoration(
//                 hintText: 'Reference #',
//                 hintStyle: TextStyle(fontSize: 11, color: Color(0xFFB0B7C3)),
//                 border: InputBorder.none,
//                 isDense: true,
//                 contentPadding: EdgeInsets.zero,
//               ),
...
//   }
//
//   Widget _buildMetaFieldCompact({required String label, required String value, required IconData icon, required VoidCallback onTap}) {
//     return GestureDetector(
//       onTap: onTap,
//       child: Container(
...
//   }
//
//   Widget _buildInvoiceItemsTableCompact() {
//     final isSaryaMode = _selectedSaleType == SaleType.sarya;
//
//     return Container(
//       decoration: BoxDecoration(
...
//             child: Row(
//               children: [
//                 const Text('Items', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
//                 const Spacer(),
//                 Container(
//                   padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
//                   decoration: BoxDecoration(color: isSaryaMode ? const Color(0xFFEFF6FF) : const Color(0xFFF0FDF4), borderRadius: BorderRadius.circular(4)),
//                   child: Row(
//                     children: [
//                       Icon(isSaryaMode ? Icons.scale : Icons.production_quantity_limits, size: 10, color: isSaryaMode ? const Color(0xFF3B82F6) : const Color(0xFF10B981)),
//                       const SizedBox(width: 2),
//                       Text(isSaryaMode ? 'Weight' : 'Qty', style: TextStyle(fontSize: 8, fontWeight: FontWeight.w600, color: isSaryaMode ? const Color(0xFF3B82F6) : const Color(0xFF10B981))),
//                     ],
//                   ),
//                 ),
//               ],
...
//             const Padding(
//               padding: EdgeInsets.all(12),
//               child: Center(child: Text('No items added', style: TextStyle(fontSize: 12, color: Color(0xFF9CA3AF)))),
//             )
//           else
//             ListView.separated(
//               shrinkWrap: true,
...
//                           crossAxisAlignment: CrossAxisAlignment.start,
//                           children: [
//                             Text('${i + 1}. ${item.product.itemName}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500)),
//                             if (item.usingCustomerPrice && item.hasPriceDifference)
//                               Text('${_selectedCustomer?.name ?? ''} price', style: const TextStyle(fontSize: 8, color: Color(0xFF10B981))),
//                           ],
//                         ),
//                       ),
//                       Expanded(
//                         child: isSaryaMode
//                             ? _buildInvoiceWeightFieldCompact(i)
//                             : Text('${item.quantity}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 11)),
//                       ),
//                       Expanded(
//                         child: Text('Rs ${item.unitPrice.toStringAsFixed(2)}', textAlign: TextAlign.right, style: TextStyle(fontSize: 11, color: item.usingCustomerPrice ? const Color(0xFF10B981) : null)),
//                       ),
//                       Expanded(
//                         child: Text('Rs ${item.totalForMode(isSaryaMode).toStringAsFixed(2)}', textAlign: TextAlign.right,
//                             style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: item.usingCustomerPrice ? const Color(0xFF10B981) : const Color(0xFF7C3AED))),
//                       ),
//                       IconButton(
//                         icon: const Icon(Icons.delete_outline, size: 14, color: Color(0xFFEF4444)),
//                         padding: EdgeInsets.zero,
...
//   }
//
//   Widget _buildInvoiceWeightFieldCompact(int index) {
//     final item = _cartItems[index];
//     return Row(
//       mainAxisAlignment: MainAxisAlignment.center,
//       children: [
//         _invoiceWeightBtnCompact(Icons.remove, () {
//           setState(() {
//             final newWeight = (item.weight - 0.5).clamp(0.0, 9999.0);
//             final old = _cartItems[index];
//             _cartItems[index] = SaleItem(
...
//           ),
//         ),
//         _invoiceWeightBtnCompact(Icons.add, () {
//           setState(() {
//             final newWeight = (item.weight + 0.5).clamp(0.0, 9999.0);
//             final old = _cartItems[index];
//             _cartItems[index] = SaleItem(
...
//   }
//
//   Widget _invoiceWeightBtnCompact(IconData icon, VoidCallback onTap) => GestureDetector(
//     onTap: onTap,
//     child: Container(
//       width: 20,
//       height: 20,
//       decoration: BoxDecoration(color: const Color(0xFFEFF6FF), borderRadius: BorderRadius.circular(4)),
//       child: Icon(icon, size: 10, color: const Color(0xFF3B82F6)),
//     ),
//   );
//
//   Widget _buildInvoiceNotesCompact() {
//     return Container(
//       padding: const EdgeInsets.all(10),
//       decoration: BoxDecoration(
//         color: Colors.white,
...
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
//           const Text('Notes', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF1E1E2D))),
//           const SizedBox(height: 4),
//           TextField(
//             controller: _invoiceNoteController,
//             maxLines: 2,
...
//
//   // ── CART PANEL (WEB) ─────────────────────────
//   Widget _buildCartPanelWeb({required bool isPOS}) {
//     return Column(
//       children: [
//         _buildCustomerSectionCompact(),
//         const Divider(height: 1, color: Color(0xFFEEEEF5)),
//         Expanded(
//           child: _cartItems.isEmpty
//               ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
//             Icon(Icons.shopping_cart_outlined, size: 32, color: Color(0xFFD1D5DB)),
//             SizedBox(height: 4),
//             Text('Cart is empty', style: TextStyle(fontSize: 12, color: Color(0xFF9CA3AF))),
//           ]))
//               : ListView.builder(
//             padding: const EdgeInsets.all(6),
//             itemCount: _cartItems.length,
//             itemBuilder: (ctx, i) => isPOS ? _buildCartItem(i) : _buildCartItemForInvoice(i),
//           ),
//         ),
//         const Divider(height: 1, color: Color(0xFFEEEEF5)),
//         _buildSummarySectionWeb(isPOS),
//       ],
//     );
//   }
//
//   Widget _buildCustomerSectionCompact() {
//     return Padding(
...
//                 child: Row(
//                   children: [
//                     Icon(_selectedCustomer != null ? Icons.person : Icons.person_add_alt, size: 14, color: _selectedCustomer != null ? const Color(0xFF7C3AED) : const Color(0xFFEF4444)),
//                     const SizedBox(width: 4),
//                     Expanded(
//                       child: Text(
//                         _selectedCustomer?.name ?? 'Select Customer *',
//                         style: TextStyle(fontSize: 11, fontWeight: _selectedCustomer != null ? FontWeight.w600 : FontWeight.w400, color: _selectedCustomer != null ? const Color(0xFF7C3AED) : const Color(0xFFEF4444)),
//                         overflow: TextOverflow.ellipsis,
//                       ),
//                     ),
//                     if (_selectedCustomer != null)
//                       GestureDetector(
//                         onTap: () => setState(() { _selectedCustomer = null; _useCustomerPrices = false; _customerPriceMap = {}; _discountAmount = 0; _discountPercent = 0; for (final item in _cartItems) { item.usingCustomerPrice = false; item.customerSpecificPrice = null; item.unitPrice = item.standardPrice; } }),
...
//             children: [
//               Expanded(
//                 child: Text(item.product.itemName, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis),
//               ),
//               IconButton(
...
//                     padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
//                     decoration: BoxDecoration(color: const Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(4)),
//                     child: Text('Rs ${item.unitPrice.toStringAsFixed(2)}', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: item.usingCustomerPrice ? const Color(0xFF065F46) : const Color(0xFF7C3AED))),
//                   ),
//                 ],
//               ),
//               const Spacer(),
...
//                 decoration: BoxDecoration(color: const Color(0xFFF3F4F6), borderRadius: BorderRadius.circular(4)),
//                 child: Text(
//                   isSaryaType ? '${item.weight.toStringAsFixed(2)} Kg' : '${item.quantity} ${item.product.unit?.symbol ?? 'pcs'}',
//                   style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600),
//                 ),
//               ),
//               const SizedBox(width: 4),
//               Container(
//                 padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2),
//                 decoration: BoxDecoration(color: item.usingCustomerPrice ? const Color(0xFF10B981).withOpacity(0.1) : const Color(0xFF7C3AED).withOpacity(0.1), borderRadius: BorderRadius.circular(4)),
//                 child: Text('Rs ${item.totalForMode(isSaryaType).toStringAsFixed(2)}', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: item.usingCustomerPrice ? const Color(0xFF10B981) : const Color(0xFF7C3AED))),
//               ),
//             ],
//           ),
//         ],
...
//                     child: TextFormField(
//                       initialValue: item.unitPrice.toStringAsFixed(2),
//                       style: TextStyle(fontSize: 10, color: item.usingCustomerPrice ? const Color(0xFF065F46) : null),
//                       decoration: InputDecoration(
...
//                 label: Text(item.hasLengthCombinations ? 'Edit Lengths' : 'Select Lengths', style: const TextStyle(fontSize: 9)),
//                 style: ElevatedButton.styleFrom(
//                   backgroundColor: item.hasLengthCombinations ? const Color(0xFF10B981) : const Color(0xFF7C3AED),
//                   foregroundColor: Colors.white,
//                   padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
...
//                 final qty = item.lengthQuantities[length] ?? 1.0;
//                 return Chip(
//                   label: Text(_safeLengthLabel(length, qty), style: const TextStyle(fontSize: 8)),
//                   backgroundColor: const Color(0xFFD1FAE5),
//                   side: const BorderSide(color: Color(0xFF10B981)),
...
//   }
//
//   Widget _buildSummarySectionWeb(bool isPOS) {
//     final customCount = _cartItems.where((i) => i.usingCustomerPrice).length;
//     final isSarya = _selectedSaleType == SaleType.sarya;
//
//     return Padding(
...
//       child: Column(
//         children: [
//           _summaryRowCompact('Subtotal', 'Rs ${_subtotal.toStringAsFixed(2)}'),
//           if (_discountValue > 0) _summaryRowCompact('Discount', '- Rs ${_discountValue.toStringAsFixed(2)}', color: const Color(0xFF10B981)),
//           const Divider(height: 8),
//           _summaryRowCompact('Total', 'Rs ${_grandTotal.toStringAsFixed(2)}', isBold: true, fontSize: 14),
//           const SizedBox(height: 8),
//           if (_cartItems.isNotEmpty && _selectedCustomer != null) ...[
//             OutlinedButton.icon(
//               onPressed: _showQuickPrintPreview,
//               icon: const Icon(Icons.print_outlined, size: 14),
//               label: const Text('Print Preview', style: TextStyle(fontSize: 11)),
//               style: OutlinedButton.styleFrom(
//                 foregroundColor: const Color(0xFF7C3AED),
//                 side: const BorderSide(color: Color(0xFF7C3AED)),
//                 shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
...
//               label: const Text('Charge', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
//               style: ElevatedButton.styleFrom(
//                 backgroundColor: const Color(0xFF7C3AED),
//                 foregroundColor: Colors.white,
//                 padding: const EdgeInsets.symmetric(vertical: 8),
...
//               label: const Text('Create Invoice', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
//               style: ElevatedButton.styleFrom(
//                 backgroundColor: const Color(0xFF7C3AED),
//                 foregroundColor: Colors.white,
//                 padding: const EdgeInsets.symmetric(vertical: 8),
...
//   }
//
//   Widget _summaryRowCompact(String label, String value, {bool isBold = false, double fontSize = 11, Color? color}) {
//     return Padding(
//       padding: const EdgeInsets.symmetric(vertical: 1),
//       child: Row(
//         children: [
//           Text(label, style: TextStyle(fontSize: fontSize, color: color ?? const Color(0xFF6B7280))),
//           const Spacer(),
//           Text(value, style: TextStyle(fontSize: fontSize, fontWeight: isBold ? FontWeight.bold : FontWeight.w600, color: color ?? const Color(0xFF1E1E2D))),
//         ],
//       ),
//     );
//   }
...
//             ),
//             const SizedBox(height: 8),
//             const Text('Use scanner or enter manually', textAlign: TextAlign.center, style: TextStyle(fontSize: 11, color: Color(0xFF6B7280))),
//             const SizedBox(height: 8),
//             TextField(
//               autofocus: true,
//               decoration: InputDecoration(
...
//     if (_selectedCustomer == null) {
//       ScaffoldMessenger.of(context).showSnackBar(
//         const SnackBar(content: Text('Please select a customer'), backgroundColor: Color(0xFFEF4444)),
//       );
//       return;
//     }
//     showDialog(
...
//       builder: (ctx) => AlertDialog(
//         shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
//         title: const Text('Document Generated', style: TextStyle(fontSize: 16)),
//         content: Column(
//           mainAxisSize: MainAxisSize.min,
//           children: [
...
//             const SizedBox(height: 8),
//             const Text('Created successfully!', textAlign: TextAlign.center, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
//             const SizedBox(height: 4),
//             Text(invoiceNumber, style: const TextStyle(fontSize: 12, color: Color(0xFF7C3AED))),
//           ],
//         ),
//         actions: [
//           TextButton(onPressed: () { Navigator.pop(ctx); SalePdfGenerator.sharePdf(pdfData, '$invoiceNumber.pdf'); }, child: const Text('Share', style: TextStyle(fontSize: 12))),
//           ElevatedButton(
...
//             Row(
//               children: [
//                 Expanded(child: _buildPrintOptionCompact(icon: Icons.print, label: 'Print', color: const Color(0xFF7C3AED), onTap: () { Navigator.pop(ctx); SalePdfGenerator.printPdf(pdfData); })),
//                 const SizedBox(width: 8),
//                 Expanded(child: _buildPrintOptionCompact(icon: Icons.share, label: 'Share', color: const Color(0xFF10B981), onTap: () { Navigator.pop(ctx); SalePdfGenerator.sharePdf(pdfData, 'receipt.pdf'); })),
//                 const SizedBox(width: 8),
//                 Expanded(child: _buildPrintOptionCompact(icon: Icons.visibility, label: 'Preview', color: const Color(0xFF3B82F6), onTap: () { Navigator.pop(ctx); _showPdfPreview(pdfData); })),
//               ],
//             ),
//             const SizedBox(height: 8),
//             TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel',style: TextStyle(fontSize: 12))),
...
//       'selected_lengths': item.selectedLengths,
//       'lengths_display': item.lengthsDisplay,
//       'length_quantities': Map<String, dynamic>.fromEntries(item.lengthQuantities.entries.map((e) => MapEntry(e.key.toString(), e.value))),
//       'weight': item.weight,
//     }).toList();
//
//     try {
...
//       final pdfData = await SalePdfGenerator.generateSalePdf(
//         saleData: {'invoice_number': 'PREVIEW-${DateTime.now().millisecondsSinceEpoch}', 'reference': _referenceController.text.trim()},
//         customer: _selectedCustomer,
//         items: items,
//         subtotal: _subtotal,
//         discountValue: _discountValue,
...
//     } catch (e) {
//       if (mounted) Navigator.pop(context);
//       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to generate preview: $e'), backgroundColor: Colors.red));
//     }
//   }
//
//   // ── SUBMIT ─────────────────────────────────────
...
//       if (missing.isNotEmpty) {
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(content: Text('SARYA mode: ${missing.length} item(s) missing weight'), backgroundColor: const Color(0xFFEF4444)),
//         );
//         return;
//       }
//     }
...
//       'discount_value': _usePercentDiscount ? _discountPercent : _discountAmount,
//       'items': _cartItems.map((item) {
//         final Map<String, dynamic> d = {'product_id': item.product.id, 'unit_price': item.unitPrice, 'used_customer_price': item.usingCustomerPrice};
//         if (isSarya) { d['weight'] = item.weight; d['quantity'] = 0; } else { d['quantity'] = item.quantity; if (item.weight > 0) d['weight'] = item.weight; }
//         if (item.selectedLengths.isNotEmpty) d['selected_lengths'] = item.selectedLengths;
//         if (item.lengthQuantities.isNotEmpty) {
//           d['length_quantities'] = Map<String, dynamic>.fromEntries(item.lengthQuantities.entries.map((e) => MapEntry(e.key.toString(), e.value)));
//         }
//         return d;
//       }).toList(),
//     };
...
//     if (mounted) Navigator.pop(context);
//     if (result['success'] == true) {
//       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Sale updated successfully'), backgroundColor: Color(0xFF10B981)));
//       Navigator.pop(context, true);
//     } else {
//       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result['message'] ?? 'Failed to update sale'), backgroundColor: const Color(0xFFEF4444)));
//     }
//   }
//
...
//       if (itemsWithoutWeight.isNotEmpty) {
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(content: Text('SARYA mode requires weight for all items. ${itemsWithoutWeight.length} item(s) have missing weight.'), backgroundColor: const Color(0xFFEF4444)),
//         );
//         return;
//       }
//     }
...
//       'items': _cartItems.map((item) {
//         final Map<String, dynamic> itemData = {'product_id': item.product.id, 'unit_price': item.unitPrice, 'used_customer_price': item.usingCustomerPrice};
//         if (_selectedSaleType == SaleType.sarya) { itemData['weight'] = item.weight; itemData['quantity'] = 0; } else { itemData['quantity'] = item.quantity; if (item.weight > 0) itemData['weight'] = item.weight; }
//         if (item.selectedLengths.isNotEmpty) itemData['selected_lengths'] = item.selectedLengths;
//         if (item.lengthQuantities.isNotEmpty) {
//           itemData['length_quantities'] = Map<String, dynamic>.fromEntries(item.lengthQuantities.entries.map((e) => MapEntry(e.key.toString(), e.value)));
//         }
//         return itemData;
//       }).toList(),
//       'discount_type': _usePercentDiscount ? 'percent' : 'fixed',
...
//         'lengths_display': item.lengthsDisplay,
//         'weight': item.weight,
//         'total': _selectedSaleType == SaleType.sarya ? item.weight * item.unitPrice : item.quantity * item.unitPrice,
//       }).toList();
//
//       try {
//         final pdfData = await SalePdfGenerator.generateSalePdf(
//           saleData: {'invoice_number': invoiceNumber, 'reference': _referenceController.text.trim()},
//           customer: _selectedCustomer,
//           items: items,
//           subtotal: _subtotal,
//           discountValue: _discountValue,
...
//           _clearCart();
//           ScaffoldMessenger.of(context).showSnackBar(SnackBar(
//             content: Text(isCredit ? 'Credit sale created!' : 'Sale completed!'),
...
//         if (mounted) {
//           _clearCart();
//           ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Sale saved but PDF failed: $e'), backgroundColor: Colors.orange));
//           Navigator.pop(context, true);
//         }
//       }
//     } else {
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(result['message'] ?? 'Failed to save sale'), backgroundColor: const Color(0xFFEF4444)));
//       }
//     }
//   }
//
...
// }
//
// // ── REST OF THE CLASSES (CustomerPickerSheet, AddCustomerDialog, PaymentDialog) ──
// // These remain unchanged but with compact styling applied
//
//
//
//
