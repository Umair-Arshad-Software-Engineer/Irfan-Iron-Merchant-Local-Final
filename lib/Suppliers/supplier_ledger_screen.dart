...
class SupplierLedgerScreen extends StatefulWidget {
  ...
  const SupplierLedgerScreen({super.key, required this.supplier, required this.languageProvider});
...
class _SupplierLedgerScreenState extends State<SupplierLedgerScreen> {
  ...
  // Payment method meta with bilingual labels
  Map<String, Map<String, dynamic>> _getPaymentMethodMeta(LanguageProvider lp) => {
  ...
  // Helper method to get payment method label
  String _getPaymentMethodLabel(String? method, Map<String, Map<String, dynamic>> meta) {
    if (method == null) return '—';
    return meta[method]?['label'] ?? method;
  }

  // Helper method to get payment method color
  Color _getPaymentMethodColor(String? method, Map<String, Map<String, dynamic>> meta) {
    if (method == null) return const Color(0xFF8E8E93);
    return meta[method]?['color'] ?? const Color(0xFF8E8E93);
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<LanguageProvider>(
      builder: (context, languageProvider, _) {
        ...
        final paymentMethodMeta = _getPaymentMethodMeta(languageProvider);
        ...
        return Scaffold(
          ...
          body: Consumer<SupplierLedgerProvider>(
            builder: (context, provider, _) => Column(children: [
              ...
              Expanded(child: _buildTableSection(provider, languageProvider, paymentMethodMeta)),
            ]),
          ),
          floatingActionButton: FloatingActionButton.extended(
  ...
  PreferredSizeWidget _buildAppBar(LanguageProvider lp) {
    return AppBar(
      ...
      title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(widget.supplier.name,
            style: const TextStyle(color: Color(0xFF1C1C1E), fontWeight: FontWeight.bold, fontSize: 17)),
        Text(lp.isEnglish ? 'Supplier Ledger' : 'سپلائر لیجر',
      ...
      actions: [
        IconButton(
          ...
          tooltip: _showUnclearedCheques
              ? (lp.isEnglish ? 'Hide uncleared cheques' : 'غیر کلیئر شدہ چیک چھپائیں')
              : (lp.isEnglish ? 'Show uncleared cheques (Admin)' : 'غیر کلیئر شدہ چیک دکھائیں (ایڈمن)'),
          onPressed: _toggleUnclearedCheques,
        ),
        IconButton(
          icon: const Icon(Icons.picture_as_pdf, color: Color(0xFF7C3AED)),
          onPressed: _generatePDF,
          tooltip: lp.isEnglish ? 'Export as PDF' : 'بطور PDF ایکسپورٹ کریں',
        ),
        IconButton(
          icon: const Icon(Icons.date_range_outlined, color: Color(0xFF7C3AED)),
          onPressed: _pickDateRange,
  ...
  Widget _summaryCard({required String label, required String value, required IconData icon,
    required Color color, required Color bgColor, bool isBold = false, required LanguageProvider lp}) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 6),
            Expanded(child: Text(label, style: TextStyle(fontSize: 11, color: Colors.grey[500],
                fontWeight: FontWeight.w500, fontFamily: lp.fontFamily), overflow: TextOverflow.ellipsis)),
          ]),
          const SizedBox(height: 8),
          Text(value, style: TextStyle(fontSize: 14,
              fontWeight: isBold ? FontWeight.bold : FontWeight.w600,
  ...
  Widget _buildTableSection(SupplierLedgerProvider provider, LanguageProvider lp, Map<String, Map<String, dynamic>> paymentMethodMeta) {
    if (provider.isLoading && provider.entries.isEmpty) {
      return const Center(child: CircularProgressIndicator(color: Color(0xFF7C3AED)));
    }
    if (!provider.isLoading && provider.entries.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text(lp.isEnglish ? 'No transactions found' : 'کوئی لین دین نہیں ملا',
            style: TextStyle(fontSize: 16, color: Colors.grey[500],
                fontWeight: FontWeight.w500, fontFamily: lp.fontFamily)),
        const SizedBox(height: 6),
    ...
    await showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              _dlgField(ctrl: descCtrl, label: lp.isEnglish ? 'Description *' : 'تفصیل *',
                  hint: lp.isEnglish ? 'e.g. Opening balance' : 'مثال: ابتدائی بیلنس', lp: lp),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: _dlgField(ctrl: creditCtrl, label: lp.isEnglish ? 'Credit (Payable)' : 'کریڈٹ (قابل ادائیگی)',
                    hint: '0.00', num: true, lp: lp)),
                const SizedBox(width: 12),
                Expanded(child: _dlgField(ctrl: debitCtrl, label: lp.isEnglish ? 'Debit (Paid)' : 'ڈیبٹ (ادا شدہ)',
                    hint: '0.00', num: true, lp: lp)),
              ]),
              const SizedBox(height: 12),
              _dlgField(ctrl: refCtrl, label: lp.isEnglish ? 'Reference # (optional)' : 'حوالہ نمبر (اختیاری)',
  ...
  Widget _dlgField({required TextEditingController ctrl, required String label,
    required String hint, bool num = false, required LanguageProvider lp}) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(fontSize: 12, color: Color(0xFF8E8E93), fontWeight: FontWeight.w500),
      ),
      const SizedBox(height: 4),
  ...
  Future<void> _generatePDF() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${lp.isEnglish ? 'Error generating PDF' : 'PDF بنانے میں خرابی'}: ${e.toString()}'),
            backgroundColor: Colors.red,
          ),
        );
      }
...
class _ExpandableRow extends StatelessWidget {
  ...
  const _ExpandableRow({
    ...
    required this.isLast, required this.currencyFormat, required this.dateFormat,
    required this.dateTimeFormat, required this.paymentMethodMeta, required this.onTap,
    this.receiptItems, this.isLoadingItems = false, required this.languageProvider,
  });
  ...
  Widget build(BuildContext context) {
    ...
    switch (entry.referenceType) {
      case 'purchase_receipt':
        ...
        typeLabel = languageProvider.isEnglish ? 'Receipt' : 'رسید';
        break;
      case 'payment':
        typeColor = const Color(0xFF10B981); typeBg = const Color(0xFFECFDF5);
      ...
      case 'reversal':
        ...
        typeLabel = languageProvider.isEnglish ? 'Reversal' : 'واپسی';
        break;
      default:
        typeColor = const Color(0xFF6366F1); typeBg = const Color(0xFFEEF2FF);
        typeIcon = Icons.edit_note_outlined;
        typeLabel = languageProvider.isEnglish ? 'Manual' : 'دستی';
    }
        ...
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(children: [
              ...
              Expanded(flex: 1, child: _methodBadge(_getPaymentMethodLabel(entry.paymentMethod),
                  _getPaymentMethodColor(entry.paymentMethod))),
              Expanded(flex: 2, child: _buildBankWidget(_getPaymentMethodColor(entry.paymentMethod))),
              Expanded(flex: 3, child: Padding(
                padding: const EdgeInsets.only(left: 4),
                child: Text(entry.description ?? '—',
                    style: const TextStyle(fontSize: 12, color: Color(0xFF1C1C1E)),
  ...
  Widget _methodBadge(String label, Color color) {
    if (label == '—') {
      return Container(
        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
        decoration: BoxDecoration(color: const Color(0xFFF5F5F7), borderRadius: BorderRadius.circular(6)),
        child: Text(label, style: TextStyle(fontSize: 10, color: const Color(0xFF8E8E93), fontWeight: FontWeight.w500,
            fontFamily: languageProvider.fontFamily),
            overflow: TextOverflow.ellipsis),
      );
    }
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: _detailItem(icon: Icons.calendar_today_outlined,
                  ...
                  value: dateTimeFormat.format(entry.transactionDate))),
              const SizedBox(width: 16),
              Expanded(child: _detailItem(icon: Icons.access_time_outlined,
                  label: languageProvider.isEnglish ? 'Recorded On' : 'ریکارڈ شدہ',
                  value: dateTimeFormat.format(entry.createdAt))),
            ]),
            const SizedBox(height: 12),
            ...
            Row(children: [
              Expanded(child: _detailItem(icon: Icons.tag_outlined,
                  ...
                  value: entry.referenceNumber ?? 'N/A')),
              const SizedBox(width: 16),
              Expanded(child: _detailItem(icon: Icons.category_outlined,
            ...
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFFF5F5F7),
                  borderRadius: BorderRadius.circular(8)),
              child: Row(children: [
                Expanded(child: _amountCell(label: languageProvider.isEnglish ? 'Debit (Paid)' : 'ڈیبٹ (ادا شدہ)',
                    value: entry.debit > 0 ? currencyFormat.format(entry.debit) : '—',
                    color: const Color(0xFF10B981))),
                Container(width: 1, height: 36, color: const Color(0xFFE5E5EA)),
                Expanded(child: _amountCell(label: languageProvider.isEnglish ? 'Credit (Payable)' : 'کریڈٹ (قابل ادائیگی)',
                    value: entry.credit > 0 ? currencyFormat.format(entry.credit) : '—',
                    color: const Color(0xFFEF4444))),
                Container(width: 1, height: 36, color: const Color(0xFFE5E5EA)),
                Expanded(child: _amountCell(label: languageProvider.isEnglish ? 'Running Balance' : 'چلتا بیلنس',
                    value: currencyFormat.format(entry.balance),
                    color: entry.balance > 0 ? const Color(0xFFEF4444)
                        : entry.balance < 0 ? const Color(0xFF10B981) : const Color(0xFF8E8E93),
                    bold: true)),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(meta['icon'] as IconData, size: 12, color: color),
              const SizedBox(width: 5),
              Text(meta['label'] as String, style: TextStyle(fontSize: 11,
                  fontWeight: FontWeight.bold, color: color, letterSpacing: 0.3,
                  fontFamily: languageProvider.fontFamily)),
            ]),
          ),
        ...
        if (chequeNum != null && chequeNum.isNotEmpty) ...[
          ...
          _infoChip(
            ...
            value: entry.chequeCleared == true
  ...
  Widget _detailItem({required IconData icon, required String label,
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(width: 6),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 10, color: const Color(0xFF8E8E93), fontWeight: FontWeight.w500,
            fontFamily: languageProvider.fontFamily)),
        const SizedBox(height: 2),
        Text(value, style: TextStyle(fontSize: 13,
            color: valueColor ?? const Color(0xFF1C1C1E),
